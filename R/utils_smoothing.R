...
smoothingBackward <- function(models, vBlocks, sampledNet, sampling, covarMatrix, covarSimilarity, split_fn, mc.cores, iter_both, control) {
  cat("   Going backward ")
  for (i in rev(vBlocks[-1])) {
    ...
    if (nlevels(cl0) == i) {
      candidates <- mclapply(combn(i, 2, simplify = FALSE), function(couple) {
        ...
        model <- missingSBM_fit$new(sampledNet, i - 1, sampling, cl_fusion, covarMatrix, covarSimilarity)
        model$doVEM(control)
        model
      }, mc.cores = mc.cores)
...
smoothingForward <- function(models, vBlocks, sampledNet, sampling, covarMatrix, covarSimilarity, split_fn, mc.cores, iter_both, control) {
  cat("   Going forward ")
    ...
    if (nlevels(cl_split) - 1 == i) {
      candidates <- mclapply(1:i, function(j) {
        ...
        if(length(cl[indices]) > 10){
          # sampledNetBis <- sampledNet$adjMatrix
          # sampledNetBis[is.na(sampledNetBis)] <- mean(sampledNetBis[!is.na(sampledNetBis)])
          cut <- as.numeric(split_fn(sampledNet$adjMatrix[indices, indices],2))
          ...
          model <- missingSBM_fit$new(sampledNet, i + 1, sampling, cl, covarMatrix, covarSimilarity)
          model$doVEM(control)
          model
        } else {
          models[[i + 1]]
...
smoothingForBackWard <- function(models, vBlocks, sampledNet, sampling, covarMatrix, covarSimilarity, split_fn, mc.cores, iter_both, control){
  out <- models
  for (i in 1: iter_both) {
    out <- smoothingForward(out, vBlocks, sampledNet, sampling, covarMatrix, covarSimilarity, split_fn, mc.cores, iter_both, control)
    out <- smoothingBackward(out, vBlocks, sampledNet, sampling, covarMatrix, covarSimilarity, split_fn, mc.cores, iter_both, control)
  }
  out
}
