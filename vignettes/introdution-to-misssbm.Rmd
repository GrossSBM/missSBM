...
```{r inferencew}
vBlocks <- 1:6
collection_sbm <- missSBM::estimate(Awarspld, vBlocks, sampling,clusterInit = "spectral", cores = 2)
```
```{r plotICL}
...
```{r inferencewfull}
vBlocks <- 1:6
collection_sbm_full <- missSBM::estimate(Awar, vBlocks, sampling, clusterInit = "hierarchical", cores = 1, trace = TRUE)
