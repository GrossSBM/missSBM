...
sampledNetwar <- missSBM::sample(Awar, sampling, samplingParameters)
...
```{r inferencew}
vBlocks <- 1:6
sbm <- missSBM::estimate(Awarspld, vBlocks, sampling,clusterInit = "spectral", smoothing = "none", mc.cores = 2)
```
```{r plotICL}
plot(ICL(sbm))
```
...
```{r inferencewfull}
vBlocks <- 1:6
sbmfull <- missSBM::estimate(Awar, vBlocks, sampling, clusterInit = "spectral", smoothing = "none", mc.cores = 2)
ICL(sbmfull)
```
