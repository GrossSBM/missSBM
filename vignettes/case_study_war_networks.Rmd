...
```{r sampling node}
partlyObservedNet_war <- missSBM::sample(belligerent_adjacency, sampling = "node", parameters = .8)
...
vBlocks <- 1:5
collection_sbm <- missSBM::estimate(partlyObservedNet_war, vBlocks = vBlocks, sampling = "node")
...
Indeed, two classes found on the fully observed network fuse in the SBM fitted on the partly observed network.
