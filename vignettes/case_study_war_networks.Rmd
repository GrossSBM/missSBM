...
We now show how to introduce covariates in the model.
...
The covariate provided as a vector is transferred on edge through an $\ell_1$ similarity: for edge $(i,j)$ the associated covariate is defined by $|x_i-x_j|$ where $x_i$ denotes the covariate for node $i$. Another similarity measure could be provided via the option `similarity`.
...
sampleNet_power_miss <- missSBM::sample(
   ...
   sampling = "covar-node",
   parameters = parameters_sample, covariates = list(war$beligerent$power), intercept = -2
  )
boxplot(1/(1 + exp(-cbind(1,war$beligerent$power) %*% c(-2, parameters_sample))) ~ sampleNet_power_miss$observedNodes, ylab="mil power",xlab = "observed node")
