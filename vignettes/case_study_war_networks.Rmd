...
```{r sampling node}
sampledNet_war <- missSBM::sample(belligerent_adjacency, sampling = "node", parameters = .8)
corrplot(sampledNet_war, 
  is.corr      = FALSE,
  tl.pos       = "n",
  method       = "color", 
  cl.pos       = "n",
...
par(mfrow = c(2,2))
plot(collection_sbm$bestModel$fittedSBM, type = "data")
plot(collection_sbm$bestModel$fittedSBM, type = "expected")
plot(collection_sbm_full$bestModel$fittedSBM, type = "data")
plot(collection_sbm_full$bestModel$fittedSBM, type = "expected")
```
...
vBlocks <- 1:4
collection_sbm_power_full <- estimate(belligerent_adjacency, vBlocks = vBlocks, sampling = "node", covariates = list(belligerent_power))
...
collection_sbm_power_miss <- estimate(sampleNet_power_miss, vBlocks = vBlocks, sampling = "covar-node", covariates = list(belligerent_power))
...
```{r samptrade}
parameters_sample <- 1
sampleNet_trade_miss <- missSBM::sample(belligerent_adjacency, sampling = "covar-dyad", parameters = parameters_sample, covariates = list(trade), intercept = -2)
...
```{r estimate trade}
collection_sbm_trade_miss <- estimate(sampleNet_trade_miss ,vBlocks = vBlocks, sampling = "covar-dyad", covariates = list(trade))
