...
collection_sbm_cov_full <- estimate(sampleNet_cov, vBlocks = 1:5, sampling = "dyad") 
smooth(collection_sbm_cov_full) 
betacov <- collection_sbm_cov_full$bestModel$fittedSBM$covarParam
```

<!-- ```{r} -->
<!-- cl0 <- missSBM:::init_clustering(sampleNet_cov$adjacencyMatrix, 2, covarArray = sampleNet_cov$covarArray, clusterInit = "spectral") -->
<!-- fit <- missSBM:::SBM_fit_covariates$new(sampleNet_cov$adjacencyMatrix, cl0, sampleNet_cov$covarArray) -->
<!-- fit$doVEM() -->
<!-- ``` -->

The military power seems to be a covariate with little influence since its effect is estimated to `r betacov`.
...
```
The military power seems to be a covariate with little influence since its effect is estimated to
`r betacov2`.


Comparison with blockmodels
```{r covblockmodel}
library(blockmodels)
modcov = BM_bernoulli_covariates_fast("SBM_sym",beligerent_adjacency,list(matsumpower),verbosity=0,plotting="")
modcov$estimate()
plot(modcov$ICL)
modcov$model_parameters[[2]]$beta
table(apply(modcov$memberships[[2]]$Z,1,which.max),collection_sbm_cov_full2$bestModel$fittedSBM$memberships)
```
